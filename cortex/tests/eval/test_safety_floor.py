...
def test_safety_floor_blocks_when_receptivity_or_confidence_fail(tmp_path):
    policy = AMIPPolicy(storage_root=str(tmp_path), n_features=8)
    x = np.ones(8, dtype=np.float64)
    ...
    assert blocked.probabilities["no_action"] == 1.0

    low_conf = policy.choose_action(
        x,
        confidence=0.2,
        receptive=True,
        stress_ratio=0.5,
    )
    assert low_conf.probabilities["no_action"] == 1.0
