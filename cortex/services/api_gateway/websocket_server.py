    ...
    async def _broadcast(self, msg: WSMessage) -> int:
        ...
        for client_id, client in self._clients.items():
            if target_types and client.client_type not in target_types:
                continue
            # Debt-2 (audit): never broadcast state to a peer that has
            # not completed the AUTH handshake. A connection in
            # ``pending_auth`` should not see STATE_UPDATE / INTERVENTION
            # frames; the gate in ``_dispatch_message`` already drops
            # such connections on their first inbound non-AUTH frame,
            # but a peer that connects-and-listens-only would still
            # receive broadcasts without this skip.
            if not client.authenticated:
                continue
            try:
