...
class LLMCache:
    ...
    def _context_key(
        ...
        F28 (audit): the SYSTEM_PROMPT and template-body hash
        (``PROMPT_TEMPLATE_VERSION``) is folded into the key so a
        template edit invalidates cached plans on the next call instead
        of serving stale results for up to ``cache_ttl_seconds``.
        Imported lazily inside the method to avoid a circular import:
        ``prompts.py`` imports schemas from this package.
        """
        # Lazy import keeps the import graph acyclic — ``prompts`` does
        # not depend on cache at import time, only at call time.
        from cortex.services.llm_engine.prompts import PROMPT_TEMPLATE_VERSION
