...
describe("F07b + F08b extension auth wiring", () => {
  ...
  it("getAuthToken hits native host once, caches in session", async () => {
    ...
    fake.runtime.sendNativeMessage.mockImplementation(
      (_app: string, _msg: unknown, cb?: (resp: unknown) => void) => {
        const response = {
  ...
  it("STOP_CORTEX attaches auth_token to SHUTDOWN payload and X-Cortex-Auth-Token to /stop", async () => {
    ...
    try {
      ...
      listener({ type: "STOP_CORTEX" }, undefined, (r) => responses.push(r));
      ...
      const shutdownFrame = sock!.sent
        .map(
          (s) =>
            JSON.parse(s) as { type: string; payload: Record<string, unknown> },
        )
        .find((f) => f.type === "SHUTDOWN");
      expect(shutdownFrame).toBeDefined();
      expect(shutdownFrame!.payload.auth_token).toBe("ab".repeat(32));

      const stopFetch = fetchCalls.find((c) => c.url.includes("/stop"));
      expect(stopFetch).toBeDefined();
      const headers =
        (stopFetch!.init?.headers as Record<string, string>) || {};
      expect(headers["X-Cortex-Auth-Token"]).toBe("ab".repeat(32));
      ...
      expect(shutdownHeaders["X-Cortex-Auth-Token"]).toBe("ab".repeat(32));
    } finally {
