...
describe("C1 — /api/cost token gating", () => {
  ...
  it("GET_COST attaches X-Cortex-Auth-Token from the cached token", async () => {
    ...
    try {
      ...
      listener({ type: "GET_COST" }, undefined, (r) => responses.push(r));
      ...
      const costFetch = fetchCalls.find((c) => c.url.includes("/api/cost"));
      expect(costFetch).toBeDefined();
      const headers =
        (costFetch!.init?.headers as Record<string, string>) || {};
      expect(headers["X-Cortex-Auth-Token"]).toBe("c0".repeat(32));
