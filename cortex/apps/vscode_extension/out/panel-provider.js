...
exports.CortexPanelProvider = void 0;
/**
 ...
 */
class CortexPanelProvider {
    _view;
    _extensionUri;
    _wsClient;
    _currentPayload = null;
    _currentState = {};
    constructor(extensionUri, wsClient) {
        ...
        switch (message.command) {
            case "dismiss":
                if (this._currentPayload) {
                    this._wsClient.sendUserAction("dismissed", this._currentPayload.intervention_id);
                    this.clearIntervention();
                }
                break;
            case "engage":
                if (this._currentPayload) {
                    this._wsClient.sendUserAction("engaged", this._currentPayload.intervention_id);
                }
                break;
        }
    }
    ...
    _getWebviewContent() {
        ...
        if (payload) {
            const headline = this._escapeHtml(payload.headline ?? "Take a moment");
            const summary = this._escapeHtml(payload.situation_summary ?? "");
            const focus = this._escapeHtml(payload.primary_focus ?? "");
            const microSteps = payload.micro_steps ?? [];
            let stepsHtml = "";
...
exports.CortexPanelProvider = CortexPanelProvider;
