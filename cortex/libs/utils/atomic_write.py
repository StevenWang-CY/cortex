"""Atomic file writes (audit F02, F49 dependency).
...
For values written once at shutdown (session reports, baselines,
onboarding markers) this is the difference between "lose the report"
and "keep the prior known-good".
...
def atomic_write_text(
    ...
    *,
    ...
    # Write + fsync + rename. Open with low-level os APIs so fsync sees
    # a real fd; ``Path.write_text`` does not expose the descriptor.
